...
Create `test-complete-flow.sh`:
```bash
#!/bin/bash
set -e

echo "=== Restaurant Picker - Complete Flow Test ==="
...
echo "Fetching users..."
USERS=$(curl -s http://localhost:8080/api/users)
INITIATOR_ID=$(echo $USERS | jq -r '[.[] | select(.canInitiateSession == true)][0].userId')
PARTICIPANT_ID=$(echo $USERS | jq -r '[.[] | select(.canInitiateSession == false)][0].userId')

echo "Initiator: $INITIATOR_ID"
echo "Participant: $PARTICIPANT_ID"

# Create session
echo -e "\n${GREEN}1. Creating session...${NC}"
SESSION=$(curl -s -X POST http://localhost:8080/api/sessions \
  -H "Content-Type: application/json" \
  -d "{\"invitedUserIds\": [\"$INITIATOR_ID\"]}")
SESSION_ID=$(echo $SESSION | jq -r '.sessionId')
...
SUBMISSIONS=$(curl -s http://localhost:8080/api/sessions/$SESSION_ID/submissions)
echo $SUBMISSIONS | jq '.[] | {restaurantName, submittedByUserId}'

# End session
echo -e "\n${GREEN}5. Ending session and selecting winner...${NC}"
RESULT=$(curl -s -X POST http://localhost:8080/api/sessions/$SESSION_ID/end \
  -H "Content-Type: application/json" \
...
ERROR=$(curl -s -X POST http://localhost:8080/api/sessions/$SESSION_ID/join \
  -H "Content-Type: application/json" \
